...
- Data: 1997 and 2001 College Alcohol Study, 14428 and 10904 observations respectively
- Goal: 
  - How do drinking and smoking affect academic performance?
  - Do the effects change from 1997 to 2001?
- Challenges: 
...
- Model: 
  ...
  - Build SEM for 2001 
  - Build SEM for 1997 using "overlapping" predictors
...
```{r eda smookinggpa2}
dat_01 %>%
  dplyr::select(e3,e4,e8,e11:e13,f5) %>% 
  mutate(f5=f5%>%
    factor(x = .,levels = 1:9,labels =  c("A","A-","B+","B","B-","C+","C","C-","D"))) %>%
  gather(data=.,key = key,value = value,-f5) %>% 
  ggplot(aes(x=value,fill=f5)) +
...
## EDA: Relation between drinking and background 
...
higher proportion of smoker related to 
- poor health
...
## EDA: Relation between smoking and background 
...
## EDA: trend of drinking behavior from 1997 to 2001
...
dat_drigpa=data.frame(
  drink=c(x,y)%>%
  factor(x = .,levels = c("abstainer","former","infrequent","light","moderate","heavy","problem")),
  year=rep(c("97","01"),c(nrow(dat_97),nrow(dat_01)))%>%as.factor(),
  gpa=c(dat_01$f5,dat_97$f4)%>%
    factor(x = .,levels = 1:9,labels =  c("A","A-","B+","B","B-","C+","C","C-","D")))%>%na.omit()
ggplot(data=dat_drigpa)+geom_mosaic(aes(x=product(drink,gpa),fill=drink))+
  facet_grid(year~.)
```

## EDA: trend of smoking behavior from 1997 to 2001

- more proportion of smokers in 1997, stronger relation to GPA in 2001
...
- Obtain the complete cases (keep 88.5%(2001), 92.4%(1997))
