...
To aggregate highly dependent features into low dimensional latent factors and study the relationship between factors, a structural equation model (SEM) is built:
...
The first component is measurement model (equation (1)), or confirmatory factor analysis (CFA), which summarize highly correlated observed variables to latent variables with taking measurement errors into consideration. We aim to construct latent variables representing drinking, smoking behavior and academic performance. The latent factor for greek life is also introduced since it may have potential influence on both drinking/smoking behavior and academic performance. 
...
The second component is strutural model (equation (2)), or structural regression model between the latent factors. We assume that "Smoking Behavior" can be explained by "Greek Life". "Drinking Behavior" can be explained by both "Greek Life" and "Smoking Behavior". "Academic" can be explained by all latent factors together wit.
