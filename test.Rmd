...
```{r final2001}
a7im=dat_01%>%dplyr::select(study_id,a7a:a7d)%>%gather(.,a7,value,-study_id)%>%na.omit()%>%filter(value>0)%>%dplyr::select(-value)%>%mutate(a7=as.factor(a7))
a7imid=dat_01%>%dplyr::select(study_id,a7a:a7d)%>%gather(.,a7,value,-study_id)%>%na.omit()%>%filter(value>0)%>%group_by(study_id)%>%summarise(n=n())%>%filter(n>1)%>%pull(study_id)
a7im=a7im%>%filter(!study_id%in%a7imid)
dat_01=left_join(dat_01,a7im,"study_id")

#here put the variables you want!!! #All data
dat_01f=dat_01%>%dplyr::select(a2,a5,a6,a8d,
                        f5,e3,e4,e8,e11,e12,
                        c7,c11,c12,problem_drinking,g15,g16,g17,
                        g3a:g3d)%>%na.omit()
dat_01f=dat_01f%>%mutate(a6i=if_else(a6==4,1,0),academic=f5,male=a2,
                         white=g3a,black=g3b,asian=g3c,native_american=g3d)
...
# sensitivity analysis on correlation matrix
...
model_un <- '
  ...
  # regressions
    ...
    drink ~ smoke
'
fit_01_non_overlap_un <- sem(model_un, data=dat_01f, ordered = c("c7","c11","c12","e3","e4","e8","academic","a5","a8d","g15","g16","g17"))
table01=summarize_lavaan(fit_01_non_overlap_un)
summary(fit_01_non_overlap_un)
model1 <- '
  # measurement model
     drink=~ c7 + c11 + c12 + problem_drinking + g15 + g16 + g17
     smoke=~ e3 + e4 + e8 + e11 + e12
  ...
  # residual correlations
   ...
   g16~~g17
'
fit_01_non_overlap1 <- sem(model1, data=dat_01f, ordered = c("c7","c11","c12","e3","e4","e8","academic","a5","a8d","g15","g16","g17"))
summarize_lavaan(fit_01_non_overlap1)
```
